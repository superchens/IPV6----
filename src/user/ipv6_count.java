...
public class ipv6_count {
	...
	public String current_new_user() throws SQLException,ClassNotFoundException {
		...
		String sql1 = "select (select count(DISTINCT user) from user_traff_ipv6 where date between "+"'"+time+"' and "+"'"+current+"' and user like '%:%') - (select count(DISTINCT user) from user_traff where date like"+"'"+time+"%' and user like '%:%') as newuser";
	...
	public String yesterday_new_user() throws SQLException,ClassNotFoundException {
		...
		String sql1 = "select (select count(DISTINCT user) from user_traff_ipv6 where date between "+"'"+time2+"' and "+"'"+time1+"' and user like '%:%') - (select count(DISTINCT user) from user_traff where date like"+"'"+time2+"%' and user like '%:%') as newuser";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public Long current_online_user() throws SQLException,ClassNotFoundException {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");  
		String current=sdf.format(new Date(d.getTime()));//获取当天的时间
		String sql1 = "select count(DISTINCT user) from user_traff_ipv6 where date like "+"'"+current+"%"+"' and user like '%:%'";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public Long yesterday_online_user() throws SQLException,ClassNotFoundException {
		...
		String sql1 = "select count(DISTINCT user) from user_traff_ipv6 where date like "+"'"+time+"%"+"' and user like '%:%'";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public String current_user() throws SQLException,ClassNotFoundException {
		...
		String current=sdf.format(new Date(d.getTime()));//获取当天的时间
        String time=sdf.format(new Date(d.getTime()- 24 * 60 * 60 * 1000));//获取前一天的时间
		String sql1 = "select (select count(DISTINCT user) from user_traff_ipv6 where date between "+"'"+time+"' and "+"'"+current+"' and user like '%:%') - (select count(DISTINCT user) from user_traff where date like"+"'"+current+"%' and user like '%:%') as olduser";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public String yesterday_user() throws SQLException,ClassNotFoundException {
		...
		String time1=sdf.format(new Date(d.getTime()- 24 * 60 * 60 * 1000));//获取当天的时间
        String time2=sdf.format(new Date(d.getTime()- 2 * 24 * 60 * 60 * 1000));//获取前一天的时间
		String sql1 = "select (select count(DISTINCT user) from user_traff_ipv6 where date between "+"'"+time2+"' and "+"'"+time1+"' and user like '%:%') - (select count(DISTINCT user) from user_traff where date like"+"'"+time1+"%' and user like '%:%') as olduser";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public Long current_user_time() throws SQLException,ClassNotFoundException {
		...
		String sql1 = "select sum(UNIX_TIMESTAMP(str_to_date(endTime,"+"'"+"%Y-%m-%d %H:%i:%s"+"'"+")) - UNIX_TIMESTAMP(str_to_date(beginTime,"+"'"+"%Y-%m-%d %H:%i:%s"+"'"+"))) sum from user_traff_ipv6 where date like "+"'"+time+"%' and user like '%:%'";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
	...
	public String current_user_traff() throws SQLException,ClassNotFoundException {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");  
		String time=sdf.format(new Date(d.getTime()));//获取当天的时间
		String sql1 = "select sum(outgoingTraff+incomingTraff) from user_traff_ipv6 where date like"+"'"+ time+"%' and user like '%:%'";
		// MySQL的JDBC URL编写方式：jdbc:mysql://主机名称：连接端口/数据库的名称?参数=值
		// 避免中文乱码要指定useUnicode和characterEncoding
		// 执行数据库操作之前要在数据库管理系统上创建一个数据库，名字自己定，
		// 下面语句之前就要先创建javademo数据库
